...
% \ELSIF{communication is {\tt Push\&Pull}}
% \STATE \hspace{5mm}\textbf{On} each node $i$:
% \STATE \hspace{10mm}${\tt Push}(\tilde{g}_{k}^{i})$

% \STATE \hspace{5mm}\textbf{On} Master:
% \STATE \hspace{10mm}$[{g}_{k}^{1},{g}_{k}^{2},{g}_{k}^{3},\cdots]=Q_{W}^{-1}([\tilde{g}_{k}^{1},\tilde{g}_{k}^{2},\tilde{g}_{k}^{3},\cdots])$
% \STATE \hspace{10mm}${g}_{k} = {\tt Allgather}([{g}_{k}^{1},{g}_{k}^{2},{g}_{k}^{3},\cdots])$
% \STATE \hspace{10mm}${\tilde{g}_{k}=Q_{M}\left(\phi_{M}\left(m_{k}, g_{k}\right)\right)}$
% \STATE \hspace{10mm}${m_{k+1}=\psi_{M}\left(m_{k}, g_{k}, \tilde{g}_{k}\right)}$

% \STATE \hspace{5mm}\textbf{On} each node $i$:
% \STATE \hspace{10mm}$\tilde{g}_{k}= {\tt Pull}(\tilde{g}_{k})$
% \STATE \hspace{10mm}${g}_{k} = Q_{M}^{-1}(\tilde{g}_{k})$
% \STATE \hspace{10mm}$x_{k+1}^{i}=x_{k}^{i}-\eta_{k}{g}_{k}$
\ENDIF
